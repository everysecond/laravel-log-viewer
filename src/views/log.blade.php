        ...
        function selectType(type) {
            ...
            if (url.indexOf('?') != '-1') {
                ...
                if(url.indexOf(param) != '-1'){
                    window.location.href = url.substring(0,url.indexOf(param)) + '&log_type=' + type;
                }else{
                    window.location.href = window.location.href + '&log_type=' + type;
                }
            } else {
                window.location.href = window.location.href + '?log_type=' + type;
            }
...
<div class="container-fluid">
    <div class="row">
        ...
        <div class="col-10 table-container">
            ...
            @else
                @if(request('max',1) && count($logs)>=1000)
                    <span style="color: red;">** 目前仅支持显示1000条，数据过大易卡，可下载后检索</span>
                    <br>
                    <br>
                @endif
                ...
                @foreach($levels as $level=>$color)
                    @if($level == request('log_type','error'))
                        <button class="nowrap text-{{$color}} selectButton selectAt"
                                onclick="selectType('{{$level}}')">{{$level}}</button>
                    @else
                        <button class="nowrap text-{{$color}} selectButton"
                                onclick="selectType('{{$level}}')">{{$level}}</button>
                    @endif
                @endforeach
                <br>
            ...
            <div class="p-3">
                @if($current_file)
                    <a href="?dl={{ \Illuminate\Support\Facades\Crypt::encrypt($current_file) }}{{ ($current_folder) ? '&f=' . \Illuminate\Support\Facades\Crypt::encrypt($current_folder) : '' }}">
                        <span class="fa fa-download"></span> 下载当前日志
                    </a>
                    -
                    <a id="clean-log"
                       href="?clean={{ \Illuminate\Support\Facades\Crypt::encrypt($current_file) }}{{ ($current_folder) ? '&f=' . \Illuminate\Support\Facades\Crypt::encrypt($current_folder) : '' }}">
                        <span class="fa fa-sync"></span> 清空当前日志
                    </a>
                    -
                    <a id="delete-log"
